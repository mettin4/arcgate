...
export default function App() {
  ...
  const C = {
    ...
    text: '#F0F0FF', muted: 'rgba(255,255,255,0.4)', purple: '#6366f1',
    purpleLight: '#a5b4fc', green: '#22c55e', yellow: '#eab308', red: '#ef4444',
  };

  const inp = { width: '100%', padding: '13px 16px', background: 'rgba(0,0,0,0.4)', border: `1px solid ${C.border}`, borderRadius: 12, fontSize: 15, color: C.text, outline: 'none', boxSizing: 'border-box', marginBottom: 14 };
      ...
      <div style={{ maxWidth: 1200, margin: '0 auto', padding: '40px 40px', display: 'grid', gridTemplateColumns: '1fr 460px', gap: 32, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div>
            ...
            <p style={{ color: C.muted, fontSize: 16, lineHeight: 1.7, margin: 0 }}>Buy and sell USDC instantly on Arc Testnet.<br/>Bank transfers in EUR, settled on-chain in seconds.</p>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
            <StatCard label="EUR → USDC Rate" value={rateLoading ? '...' : `1 EUR = ${eurRate.toFixed(4)} USDC`} sub={rateLoading ? 'Loading...' : 'Updates every 30s'}/>
          ...
          <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 20, padding: 28 }}>
            ...
            {!wallet ? (
              <div style={{ textAlign: 'center', padding: '28px 0' }}>
                ...
                <p style={{ color: '#fff', fontSize: 14, fontWeight: 600, margin: '0 0 4px' }}>Connect your wallet</p>
                <p style={{ color: C.muted, fontSize: 13, margin: 0 }}>Your transaction history will appear here.</p>
              </div>
            ) : txHistory.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '28px 0' }}>
                <div style={{ width: 48, height: 48, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.15)', borderRadius: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', fontSize: 22 }}>📋</div>
                  ...
                  {onStep === 1 && !onStatus && (
                    <>
                      ...
                      <div style={{ position: 'relative', marginBottom: 12 }}>
                        ...
                        <input type="number" placeholder="0.00" value={onAmount} onChange={e => handleOnAmount(e.target.value)} min="0"
                      ...
                      <button onClick={startOnRamp} disabled={!onAmountValid || !wallet} style={{ ...btnPrimary, opacity: onAmountValid && wallet ? 1 : 0.4 }}>Continue to Payment →</button>
                ...
                <>
                  ...
                  {offStep === 1 && !offStatus && (
                    <>
                      ...
                      <div style={{ position: 'relative', marginBottom: 12 }}>
                        <input type="number" placeholder="0.00" value={offAmount} onChange={e => handleOffAmount(e.target.value)} min="0"
                      ...
                      <button onClick={() => offAmountValid && wallet && setOffStep(2)} disabled={!offAmountValid || !wallet} style={{ ...btnPrimary, opacity: offAmountValid && wallet ? 1 : 0.4 }}>Continue →</button>
                      {!wallet && <p style={{ color: C.muted, fontSize: 12, textAlign: 'center', marginTop: 10 }}>Connect your wallet to continue</p>}
                    </>
                  )}
                  {offStep === 2 && !offStatus && (
                    <>
                      ...
                      {offIban && !validateIBAN(offIban) && <p style={{ color: C.red, fontSize: 12, margin: '-10px 0 10px' }}>Please enter a valid IBAN</p>}
                      ...
                      <button onClick={() => offName && offIban && validateIBAN(offIban) && setOffStep(3)} style={{ ...btnPrimary, opacity: offName && offIban && validateIBAN(offIban) ? 1 : 0.4 }}>Review & Confirm →</button>
                    </>
                  )}
                  {offStep === 3 && !offStatus && (
                    <>
                  ...
                  {offStatus === 'pending' && <div style={{ textAlign: 'center', padding: 20 }}><p style={{ color: C.yellow, fontSize: 15 }}>⏳ Confirming on Arc Testnet...</p></div>}
                  {offStatus === 'success' && (
                    <div style={{ textAlign: 'center' }}>
                      <div style={{ width: 72, height: 72, background: 'rgba(34,197,94,0.1)', border: '1px solid rgba(34,197,94,0.25)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', fontSize: 32 }}>✓</div>
